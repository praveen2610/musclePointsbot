...
function buildCommands() {
    ...
    const adminCategoryChoices = [...fixedPointCategories.map(c => ({name: c.charAt(0).toUpperCase() + c.slice(1), value: c})), { name: 'Exercise', value: 'exercise' }];
...
class CommandHandler {
    ...
    async handleClaim(interaction, category) {
        ...
        if (achievements.length) {
             await interaction.editReply(payload); // Send main public reply first
             return interaction.followUp({ // Send achievement privately
    ...
    async handleDistance(interaction, activity) {
        ...
        // Log with specific activity name (walking, jogging, running)
        ...
        // Commit the shared exercise cooldown
        this.db.commitCooldown({ guildId: guild.id, userId: user.id, category: cooldownKey }); 
    ...
    async handleExercise(interaction) {
        ...
        const embed = new EmbedBuilder().setColor(cur.color).setDescription(description).addFields(
                { name: "Total", value: `🏆 ${formatNumber(userRow.total)}`, inline: true },
...
async function main() {
    ...
    try { console.log('Registering...'); const route = CONFIG.devGuildId ? Routes.applicationGuildCommands(CONFIG.appId, CONFIG.devGuildId) : Routes.applicationCommands(CONFIG.appId); await rest.put(route, { body: buildCommands() }); console.log('Registered.'); }
    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        try {
            ...
            try { if (interaction.deferred || interaction.replied) await interaction.editReply(errorReply); else await interaction.reply(errorReply); } 
